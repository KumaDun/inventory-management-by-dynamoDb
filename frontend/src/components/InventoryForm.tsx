...
const InventoryForm: React.FC<InventoryFormProps> = ({ item, onClose, onSuccess }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4 z-50">
            <div className="glass-panel w-full max-w-lg p-8 animate-in fade-in zoom-in duration-200 bg-white">
                <div className="flex justify-between items-center mb-6">
                    <h2 className="text-xl font-bold text-gray-900">{item ? 'Edit Item' : 'New Item'}</h2>
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1.5">Item ID</label>
                            <input
                                name="itemId"
                    ...
                    <div>
                        ...
                        <textarea name="description" value={formData.description || ''} onChange={handleChange} className="input-field h-24 resize-none" placeholder="Enter product description..." />
                    </div>
                    ...
                    <div className="flex items-center gap-3 py-2 px-1">
                        ...
                        />
                        <label htmlFor="available" className="text-sm font-medium text-gray-700 cursor-pointer">Available for sale</label>
                    </div>

                    <div className="flex justify-end gap-3 mt-8 pt-4 border-t border-gray-100">
                        <button type="button" onClick={onClose} className="px-5 py-2 hover:bg-gray-100 rounded-lg text-gray-600 font-medium transition-colors">Cancel</button>
